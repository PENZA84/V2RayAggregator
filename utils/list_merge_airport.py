...
class sub_merge():
    def read_list(json_file, remote=False):  # Чтение списка из JSON
    ...
    def readme_update(readme_file='./README.md', sub_list=[]):  # Обновление README
        ...
        total = 0
        if os.path.exists('./sub/sub_merge.txt'):
        ...
        for repo in sub_list:
            ...
            if repo['enabled'] == True:
                ...
                # ✅ ДОБАВИЛ ПРОВЕРКУ: существует ли файл?
                if os.path.exists(sub_file):
                    with open(sub_file, 'r', encoding='utf-8') as f:
                        proxies = f.readlines()
                        if proxies == ['Url 解析错误'] or proxies == ['订阅内容解析错误'] or proxies == []:
                            amount = 0
                        else:
                            amount = len(proxies)
                        f.close()
                else:
                    amount = 0  # Если файла нет — пишем 0 узлов
        ...
        # Блок с высокоскоростными узлами
        for index in range(len(lines)):
            if lines[index] == '### high-speed node\n':
                lines.pop(index+1)
                while index+4 < len(lines) and lines[index+4] != '\n':
                    lines.pop(index+4)

                if os.path.exists('./Eternity'):
        ...
        # Блок со всеми узлами
        for index in range(len(lines)):
            if lines[index] == '### all nodes\n':
                lines.pop(index+1)

                proxies = []
                if os.path.exists('./sub/sub_merge_yaml.yml'):
...
if __name__ == '__main__':
    ...
    # Получаем подписки — если ничего не получится, файл не создастся, но код не упадёт
    subs.get_subs_v3(
    ...
    # Обновляем README — теперь с проверками на файлы
