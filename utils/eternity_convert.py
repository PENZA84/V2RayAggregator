...
def eternity_convert(file, config, output, provider_file_enabled=True):
    # Получение списка узлов — без изменений
    ...
    # Добавление информации в логи — как в оригинале
    ...
    for line in lines:
        if line != 'proxies:':
            ...
            if "password" in line_parsed:
                line_parsed["password"] = str(line_parsed["password"])
    ...
    # Сохранение провайдеров — только то что было
